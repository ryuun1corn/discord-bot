...
function getSnippet(text) {
  // Checking if there is a code snippet and get the language (if there's any)
  const languangesId = [
    "csharp",
    "c",
    "c++",
...
async function handleError(interaction, error) {
  // For logging to error channel
  const errorChannel = await interaction.client.channels.fetch(
...
function iterateCommands() {
  ...
  for (const folder of commandsFolders) {
    ...
    const commandFiles = fs
      ...
      .filter((file) => file.endsWith(".js")); // Read all the .js files inside the folder
...
function maintenanceMessage() {
  return {
    content:
      "This command is currently under maintenance. Apologies for the inconvenience.",
    ephemeral: true,
  };
}
